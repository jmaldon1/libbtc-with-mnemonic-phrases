...
btc_bool gen_privatekey(const btc_chainparams* chain, char* privkey_wif, size_t strsize_wif, char* privkey_hex_or_null, char* mnemonic)
{
    btc_key key;
    btc_privkey_init(&key);
    btc_privkey_gen(&key);
    btc_mnemonic_gen(&key, mnemonic);
...
btc_bool hd_print_node(const btc_chainparams* chain, const char* nodeser)
{
    ...
    if (!btc_hdnode_deserialize(nodeser, chain, &node))
        return false;
    char* mnemonic_phrase = mnemonic_from_data(node.private_key, BTC_ECKEY_PKEY_LENGTH);
